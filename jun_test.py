...
Major parameters.
"""
FOLDER = f'fbm_simulations/1100_0.25'  # The folder containing .h5(BI-ADD) or .csv(FreeTrace) files.
...
p6 = sns.histplot(data=analysis_data1, x='duration', stat='count', hue='state', binwidth=FRAMERATE, kde=True, palette=cmap_for_plot)
...
if tamsd is not None:
    ...
    tamsd = tamsd.dropna()
    axs.plot(tamsd['time'], curve(tamsd['mean'][1], theoretical_H, tamsd['time'].to_numpy() / FRAMERATE), label=f'Estimated Curve: K and H\n({round(tamsd['mean'].to_numpy()[1], 4)}, {round(np.mean(theoretical_H), 4)})', c='green')
    ...
    axs.plot(tamsd['time'], curve(tamsd['mean'][1], cdf_cacuhy_val, tamsd['time'].to_numpy() / FRAMERATE), label=f'CDF Curve: K and H\n({round(tamsd['mean'].to_numpy()[1], 4)}, {round(np.mean(cdf_cacuhy_val), 4)})', c='purple')
